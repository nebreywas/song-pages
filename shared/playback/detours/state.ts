...
export function resolveTrackEndAdvance(input: {
  ...
  /** When false, repeat-one on primary stops instead of looping an unplayable row. */
  currentSongPlayable?: boolean;
}): TrackEndAdvanceAction {
  const { state, repeatMode, currentSongId, currentSongPlayable = true } = input;
