...
function emptyMetadata(clipId) {
  return {
    ...
    explicit: null,
    // Provider-imported Studio counters — not Song Pages History play stats.
    sunoPlayCount: null,
    sunoLikeCount: null,
    commentCount: null,
    isInstrumental: null,
...
function parseSunoProviderMetadata(raw) {
  ...
  if (value.schemaVersion === SUNO_CLIP_METADATA_SCHEMA_VERSION && value.provider === SUNO_PROVIDER_ID) {
    ...
    return {
      ...
      sunoPlayCount: asFiniteNumber(value.sunoPlayCount) ?? asFiniteNumber(value.playCount),
      sunoLikeCount: asFiniteNumber(value.sunoLikeCount) ?? asFiniteNumber(value.upvoteCount),
